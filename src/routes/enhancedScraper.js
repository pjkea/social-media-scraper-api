...
const RealisticScrapingService = require('../services/realisticScrapingService');
        ...
        for (const sessionDir of sessionDirs) {
            try {
                ...
                if (await fs.pathExists(sessionInfoPath)) {
                    ...
                    const size = await RealisticScrapingService.getDirectorySize(sessionPath);
                    ...
                    sessions.push({
                        ...
                        size: RealisticScrapingService.formatBytes(size)
                    });
                }
            } catch (error) {
...
router.post('/sessions/cleanup', async (req, res) => {
    try {
        ...
        res.json({
            ...
            freedSpace: RealisticScrapingService.formatBytes(freedBytes),
            olderThanDays
        });
    } catch (error) {
...
// Remove the helper method attachments since they're now static methods in the service class
